...
SQLTeacher 是一款面向数据库课程教学与 SQL 练习的本地优先 Java 桌面应用。`v1.0.0` 提供 SQLite 离线教学、受限 MySQL/MariaDB 查询、确定性练习评测、学情分析、本地知识检索和可选 Ollama 辅助。
...
脚本会生成 `SQLTeacher-1.0.0.exe` 正式安装器、便携 app-image 和 `SQLTeacher-1.0.0-windows-x64.zip`。WiX 3.14.1 在首次打包时下载到 `target/tools` 并校验 SHA-256，不进入 Git。
...
`v1.0.0` 是单机本地版，不含账号、班级、云同步和多人协作。Windows 安装器暂未进行代码签名；Ollama 的生成质量取决于本地模型，服务不可用时核心练习仍可使用。详见发布说明中的已知限制。
