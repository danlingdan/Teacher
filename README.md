...
SQLTeacher 是一款面向数据库课程教学与 SQL 练习的本地优先 Java 桌面应用。`v1.2.0` 提供正式 HTTPS 云服务、安全会话恢复、可靠学习记录同步、教学任务生命周期、班级统计与审计导出，同时保留完整离线教学闭环。
...
脚本会生成 `SQLTeacher-1.2.0.exe` 正式安装器、便携 app-image 和 `SQLTeacher-1.2.0-windows-x64.zip`。WiX 3.14.1 在首次打包时下载到 `target/tools` 并校验 SHA-256，不进入 Git。
...
`v1.2.0` 的云端功能仍是可选增强；服务器或网络 AI 不可用时，SQLite 练习、确定性评测和本地知识检索仍可使用。云端地址须以 `-Dsqlteacher.cloud.base-url=https://api.sqlteacher.tech` 配置；客户端拒绝非回环 HTTP 地址，正式教学数据只能经域名与 HTTPS 传输。
