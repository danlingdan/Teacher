...
SQLTeacher 是一款面向数据库课程教学与 SQL 练习的本地优先 Java 桌面应用。`v1.1.0` 在原有离线教学闭环上增加启动身份门禁、访客/学生/教师/管理员权限、多教师班级、学习记录同步和用户自带 API Key 的可选网络 AI。
...
脚本会生成 `SQLTeacher-1.1.0.exe` 正式安装器、便携 app-image 和 `SQLTeacher-1.1.0-windows-x64.zip`。WiX 3.14.1 在首次打包时下载到 `target/tools` 并校验 SHA-256，不进入 Git。
...
`v1.1.0` 的云端功能为可选增强；服务器或网络 AI 不可用时，SQLite 练习、确定性评测和本地知识检索仍可使用。当前测试服务器以 HTTP 暂时开放，仅用于联调，正式教学数据必须在域名与 HTTPS 启用后使用。
