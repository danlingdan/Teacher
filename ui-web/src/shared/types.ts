...
export interface ActivityDefinition {
  ...
  type:
    ...
    | "READING"
    | "SQL";
  nextSubmissionVersion: number;
  latestFeedback?: { comment: string; createdAt: string };
...
export interface TeachingWorkspace {
  ...
  progressOverview: {
    ...
    averageSubmissionDuration: number | string;
    hintsUsed: number;
...
export interface ExerciseDefinition extends ExerciseSummary {
  description: string;
  datasetId: string;
  referenceSql: string;
  evaluationRule: {
...
export interface CloudAssignment {
  ...
  title: string;
  description: string;
  status: "DRAFT" | "PUBLISHED" | "CLOSED" | "WITHDRAWN" | "ARCHIVED";
  dueAt?: string;
  createdAt: string;
  updatedAt: string;
  version: number;
...
export interface AssignmentDelivery {
  operationId: string;
  status: "PASSED" | "SUBMITTED" | "QUEUED" | "REJECTED";
...
export interface SubmissionFeedback {
  ...
  status: "NEEDS_WORK" | "REVIEWED" | "RESOLVED";
  comment: string;
  knowledgePointIds: string[];
  version: number;
  authorUserId: string;
...
export interface CloudCourseSection {
  id: string;
  courseId: string;
  name: string;
  sortOrder: number;
...
export interface CloudKnowledgePoint {
  id: string;
  courseId: string;
  sectionId: string;
  name: string;
...
export interface SharedExerciseVersion {
  id: string;
  exerciseId: string;
  courseId: string;
  version: number;
...
export interface CloudCourseContent {
  sections: CloudCourseSection[];
  knowledgePoints: CloudKnowledgePoint[];
  exercises: SharedExerciseVersion[];
  cached: boolean;
}
export interface CoursePackagePreview {
  ...
  exercises: number;
  conflict: "NONE" | "SAME_CONTENT" | "VERSION_CONFLICT";
}
...
export interface SettingsPreferences {
  ...
  general: {
    ...
    supportLoggingExpiresAt: number;
    updateMirrorsEnabled: boolean;
    language: "zh" | "en";
    nativeNotificationsEnabled: boolean;
    meteredNetwork: boolean;
    theme: "system" | "light" | "dark";
    font: "modern" | "system" | "classic";
