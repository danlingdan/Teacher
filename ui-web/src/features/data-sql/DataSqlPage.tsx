...
function AiAssistant({ connectionId, onDraft }: { connectionId: string; onDraft: (sql: string) => void }) {
  ...
  return <section className="content-card ai-panel"><div className="section-heading"><div><p className="eyebrow">草稿不自动执行</p><h2>自然语言生成 SQL</h2></div>{generate.isPending && requestId && <Button variant="danger" onClick={() => void cancelLocalAppRequest(requestId)}>取消</Button>}</div><FormField label="查询目标" hint="先检查将发送的上下文，再调用当前 AI 提供方">{ids => <textarea {...ids} value={question} onChange={event => { setQuestion(event.target.value); setPreview(undefined); setResult(undefined); }} maxLength={2000} />}</FormField><div className="button-row"><Button variant="secondary" disabled={!connectionId || question.trim().length < 2 || inspect.isPending} onClick={() => inspect.mutate()}>预览 AI 上下文</Button>{preview && <Button disabled={generate.isPending} onClick={() => generate.mutate()}>确认并生成草稿</Button>}</div>{preview && <Feedback tone="info" title={`将发送 ${preview.characterCount} 个字符`}><p>类别：{preview.categories.join("、") || "无"}</p><p>来源：{preview.sources.join("、") || "无"}</p><p>脱敏：{preview.redactions.join("、") || "无需额外脱敏"}</p></Feedback>}{(inspect.isError || generate.isError) && <Feedback tone="error" title="AI SQL 未生成，数据库未执行">{(inspect.error ?? generate.error)?.message}</Feedback>}{result && <div className="ai-answer"><pre>{result.plan.sqlDraft || "（模型没有返回 SQL 草稿）"}</pre><p>{result.plan.explanation}</p><div className={`risk-strip risk-${result.riskAnalysis.level.toLowerCase()}`}><strong>{result.riskAnalysis.level} · {result.riskAnalysis.statementType}</strong><span>{result.accepted ? "可复制到工作台继续人工审查" : "Java 安全门禁未接受此草稿"}</span>{result.riskAnalysis.reasons.map(reason => <small key={reason}>{reason}</small>)}</div><Button disabled={!result.accepted} onClick={() => onDraft(result.plan.sqlDraft)}>复制到 SQL 工作台</Button></div>}</section>;
}
