...
- Keep business rules and orchestration outside JavaFX controllers.
- Keep JDBC, HTTP, Ollama, files, and JavaFX types out of domain and application contracts.
...
- Keep background database, file, network, and AI work off the JavaFX application thread. Represent loading, success, empty, and failure states honestly.
...
3. Run `mvn test` for cross-module, schema, security, release-bound, or accumulated changes. Use CLI verification apps when UI startup is unavailable; use `mvn javafx:run` only with a desktop graphics environment.
