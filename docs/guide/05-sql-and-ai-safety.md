...
- 禁止默认执行多语句；多语句拦截不受安全模式影响。唯一例外是 v3.3 练习沙盒：写操作/脚本/触发器题的提交按题型拆分、逐条门禁，在一次性隔离库上执行；禁止类语句（含脚本内部）仍然全部拦截。
...
| 等级 | 示例 | 策略 |
|---|---|---|
| 低 | `SELECT`（含 CTE `WITH ... SELECT`、括号复合查询）、SQLite `EXPLAIN`/`EXPLAIN QUERY PLAN`（v3.3 起） | 允许执行，限制行数 |
| 中 | `INSERT`、`CREATE` | 开发者模式直接执行；教学模式确认 |
| 高 | `UPDATE`、`DELETE`、`ALTER`、`DROP`、`TRUNCATE`、`GRANT`、`REVOKE`、`DROP DATABASE`、文件读取函数 | 必须最终确认 |
| 禁止 | 多语句（仅练习沙盒按题型放宽，见上）、用户/角色管理、写文件类操作（`INTO OUTFILE`、`COPY ... TO FILE/PROGRAM`） | 直接拦截 |
