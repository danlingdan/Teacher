...
> 状态：Beta 架构已冻结；公共组件与分层样式继续作为唯一新增路径

> 2026-08-09 Beta 决策：现有 `app.css` 作为只读兼容层保留，2.0 新组件和新页面不得继续向其中追加规则；
> 原 F 批次“发布前完全删除兼容层”退出条件因大范围视觉回归风险取消。token、component、page、responsive
> 分层与共享 UI Kit 仍是冻结架构，后续只做缺陷、无障碍和兼容修正。
